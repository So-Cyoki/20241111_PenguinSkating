...
namespace NWH.DWP2.WaterObjects
{
    ...
    public class WaterObject : MonoBehaviour
    {
	    ...
	    /// </summary>
	    [NonSerialized] private Mesh simulationMesh;
		...
		/// <summary>
		/// Resulting volume of buoyancy calculation.
		/// </summary>
	    ...
	    private void FixedUpdate()
        {
            ...
            if (CurrentWaterDataProvider != null)
            {
	            CurrentWaterDataProvider.GetWaterHeightsFlowsNormals(this, ref WorldVertices, ref WaterHeights, 
		            ref WaterFlows, ref WaterNormals, calculateWaterHeights, calculateWaterNormals, 
		            calculateWaterFlows);
            }
	    ...
	    public void OnExitWaterDataProvider(WaterDataProvider wdp)
        {
			...
			if (availableWaterDataProviders.Count == 0)
			{
				for (int i = 0; i < triangleCount; i++)
				{
					if (WaterHeights.Length != triangleCount) WaterHeights = new float[triangleCount];
                    if (WaterNormals.Length != triangleCount) WaterNormals = new Vector3[triangleCount];
                    if (WaterFlows.Length != triangleCount) WaterFlows = new Vector3[triangleCount];
