...
Shader "Crest/Ocean"
{
	Properties
	{
		_OverallBrightness("Overall Brightness", Range(0.01, 5.0)) = 1
		[Header(Toon Shading Settings)]
		_ToonLevels("Toon Levels", Range(2, 20)) = 3
		_ToonContrast("Toon Contrast", Range(0.0, 5.0)) = 2.0
		[Header(Toon Specular Settings)]
		_ToonSpecularIntensity("Toon Specular Intensity", Range(0.0, 5.0)) = 1.0
		_ToonSpecularSharpness("Toon Specular Sharpness", Range(0.1, 128.0)) = 16.0
		_ToonSpecularThreshold("Toon Specular Threshold", Range(0.0, 5.0)) = 0.5
		_ToonSpecularColor("Toon Specular Color", Color) = (1.0, 1.0, 1.0, 1.0)
		[Header(Normals)]
		...
		_NormalsScale("Normal Map Scale", Range(0.01, 1000.0)) = 40.0
		// Strength of normal map influence
		_NormalsStrength("Normal Map Strength", Range(0.1, 10.0)) = 5.0
		...
		[Toggle] _SubSurfaceScattering("Enable", Float) = 0
		...
		[Toggle] _Caustics("Enable", Float) = 0
		// Caustics texture
	...
	SubShader
	{
		...
		Pass
		{
			...
			uniform float _ToonSpecularSharpness;
			uniform float _ToonSpecularThreshold;
			uniform half4 _ToonSpecularColor;

			half Toonify(half intensity, int levels, half contrast)
			{
			    half step = 1.0 / levels;
			    intensity = floor(intensity / step) * step;
			    intensity = saturate((intensity - 0.5) * contrast + 0.5);
			    return intensity;
			};
			half ToonifySpecular(half intensity, half threshold)
			{
			...
			half4 Frag(const Varyings input, const bool i_isFrontFace : SV_IsFrontFace) : SV_Target
			{
    			...
    			// 计算法线与半角向量的点积
    			half NdotH = saturate(dot(n_pixel, halfDir));
						
    			// 计算镜面反射强度
    			half specularIntensity = pow(NdotH, _ToonSpecularSharpness) * _ToonSpecularIntensity;
						
    			// 使用 Toonify 函数对镜面反射进行量化
				half toonSpecular = ToonifySpecular(specularIntensity, _ToonSpecularThreshold);
						
    			// 将镜面反射添加到颜色中
    			col.rgb += toonSpecular * _ToonSpecularColor.rgb;
						
    			// 确保颜色不超过 1.0
    			col.rgb = saturate(col.rgb);

				// 将颜色转换为亮度
				half intensity = dot(col.rgb, half3(0.299, 0.587, 0.114));

				// 对亮度进行卡通化
				half toonIntensity = Toonify(intensity, _ToonLevels, _ToonContrast);

				// 避免除以零
				intensity = max(intensity, 0.001);

				// 使用卡通化的亮度调整颜色
				col.rgb = col.rgb * (toonIntensity / intensity);
				...
				col.rgb *= _OverallBrightness; // 修改整体亮度
				return half4(col, 1.);
			}
